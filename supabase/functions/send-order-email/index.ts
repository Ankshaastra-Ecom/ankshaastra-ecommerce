...
// // Google Apps Script Web App URL (Extensions -> Apps Script -> Deploy as
// // Web App, "Anyone" access) — same lightweight pattern used on the main
// // Ankshaastra Operations Console (no Google Cloud service account, no API
// // key needed). Every order pushes one row into the connected Google Sheet.
...
// /**
//  * Fire-and-forget push of one order's details into the Google Sheet.
//  * Never throws — a Sheet outage/misconfiguration must never block or fail
//  * the actual order-confirmation email, which is this function's real job.
//  */
...
//     });
//   } catch (err) {
//     // Log only — this must never break order confirmation / email sending.
//     console.error('[google-sheet] failed to push order row:', err instanceof Error ? err.message : err);
//   }
// }
...
//     // Fire-and-forget — don't await/block the email flow on this.
//     void pushOrderToGoogleSheet({
//       orderNumber,
//       customerName,
//       email: to,
...
async function buildInvoicePdfBytes(params: {
  ...
  const { orderNumber, orderDate, customerName, customerPhone, shippingCity, shippingState, shippingPincode, items, subtotal, shipping, discount, total } = params;
  ...
  // Discount is applied to the goods total, not shipping. Distribute it
  // proportionally across items so GST is charged on what was actually paid
  // for each item, not its pre-discount list price.
  const grossItemsTotal = items.reduce((sum, item) => sum + item.total, 0);
  const netGoodsTotal = Math.max(grossItemsTotal - (discount || 0), 0);
  const discountScale = grossItemsTotal > 0 ? netGoodsTotal / grossItemsTotal : 1;

  items.forEach((item, i) => {
    if (y < 120) return;
    const lineTotal = Math.round(item.total * discountScale * 100) / 100;
    const base = Math.round((lineTotal / (1 + GST_RATE)) * 100) / 100;
    const tax = Math.round((lineTotal - base) * 100) / 100;
    taxableTotal += base;
    taxTotal += tax;
  ...
  y -= 14;
  if (discount) {
    drawText('Discount', 380, y, 9);
    drawText(`-Rs.${discount.toFixed(2)}`, 520, y, 9);
    y -= 14;
  }
  drawText('Shipping', 380, y, 9);
  drawText(shipping === 0 ? 'FREE' : `Rs.${shipping.toFixed(2)}`, 520, y, 9);
...
Deno.serve(async (req) => {
  ...
  try {
    const body: OrderEmailRequest = await req.json();
    const { to, customerName, orderNumber, items, subtotal, shipping, discount, total, paymentMethod, shippingAddress } = body;
