...
const Auth: React.FC = () => {
  ...
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const { signInWithPhone, signUpWithPhone, signInWithGoogle } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        const { error, data } = await signUpWithPhone(phone, password);
        if (error) throw error;
        if (data?.user?.identities?.length === 0) {
          toast({ title: 'Account exists', description: 'An account with this mobile number already exists. Please sign in.', variant: 'destructive' });
        } else if (data?.session) {
          const referralCode = searchParams.get('ref');
          if (referralCode) {
            await supabase
          ...
          // Supabase project has "Confirm email" turned on, so signup won't
          // return a session immediately. Since we use a pseudo-email, the
          // customer can never receive that confirmation mail — this must be
