...
const OrderConfirmation: React.FC = () => {
  const { orderNumber } = useParams<{ orderNumber: string }>();
  const [order, setOrder] = useState<StoredOrder | null>(null);
  const { user, signUpWithPhone } = useAuth();
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      const { error, data } = await signUpWithPhone(order.phone, accountPassword);
      if (error) throw error;

      if (data?.session) {
        // Link only this specific just-placed order — never anything else.
        const { error: linkError } = await supabase.functions.invoke('link-guest-order', {
        ...
        toast({ title: 'Account created!', description: 'This order is now saved to your account.' });
      } else {
        // Project has email confirmation ON — pseudo-email can't receive it.
        toast({
          title: 'Almost there',
          description: 'Account created, but sign-in needs "Confirm email" turned off in Supabase Auth settings.',
        });
      }
    } catch (err: any) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-10 md:py-16">
        <div className="container-custom max-w-3xl">
          ...
          {!user && order && !accountCreated && (
            <div className="mt-6 card-spiritual p-5 md:p-6">
              <h2 className="font-semibold mb-1">Create an account to save and view your order history</h2>
              <p className="text-sm text-muted-foreground mb-4">
                Optional — using mobile number <span className="font-medium">{order.phone}</span> and a password of
                your choice.
              </p>
              <form onSubmit={handleCreateAccount} className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1">
                  <Label htmlFor="accountPassword" className="sr-only">Password</Label>
                  <Input
                    id="accountPassword"
                    type="password"
                ...
                <Button type="submit" className="btn-gold rounded-full" disabled={creatingAccount}>
          ...
          {accountCreated && (
            <div className="mt-6 card-spiritual p-5 md:p-6 text-sm text-sage font-medium">
              ✓ Account created — you can now track this order anytime from "My Orders".
            </div>
          )}
